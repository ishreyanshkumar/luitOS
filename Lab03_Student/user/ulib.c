...
/* ---- Lab 3: shared-information-page fast accessors ---- */
static volatile struct usysinfo *
usi_page(void)
{
    return (volatile struct usysinfo *)USYSINFO_VA;
}

int
u_snapshot(struct usysinfo *out)
{
    /* TODO-BEGIN U1
     * Read one consistent snapshot from USYSINFO_VA.
     * Return 0 on success. Return -1 if out is 0 or the ABI version does not match.
     *
     * Required loop:
     *   - read seq into a local variable; retry if it is odd;
     *   - use __sync_synchronize();
     *   - copy every field in struct usysinfo;
     *   - use __sync_synchronize() again;
     *   - read seq again; accept only if both values are equal and even.
     *
     * Do not call a system call on the normal compatible path.
     */
    if(out == 0) return -1;

    volatile struct usysinfo *usi = usi_page();

    if(usi->version != USYSINFO_VERSION) return -1;

    uint32 seq_start;
    do{
        seq_start = usi->seq;

        // skip if its odd maybe kernel is currently writing
        if(seq_start % 2 != 0) continue;
        __sync_synchronize();

        // copy every field in struct usysinfo
        
        out->version = usi->version;
        out->seq = usi->seq;
        out->pid = usi->pid;
        out->hart = usi->hart;
        out->ticks = usi->ticks;
        out->syscall_count = usi->syscall_count;
        out->ctxsw_count = usi->ctxsw_count;
        out->state_gen = usi->state_gen;
        
        __sync_synchronize();
    }while(seq_start != usi->seq || usi->seq % 2 != 0);
...
{
    /* TODO-BEGIN U2
     * Use u_snapshot() and return the shared-page pid.
     * If u_snapshot() returns -1, fall back to getpid().
     */
    struct usysinfo usi;
    if(u_snapshot(&usi) == 0)
        return usi.pid;
...
{
    /* TODO-BEGIN U3
     * Use u_snapshot() and return the shared-page ticks value.
     * If u_snapshot() returns -1, fall back to uptime().
     */
    struct usysinfo usi;
    if(u_snapshot(&usi) == 0)
        return (unsigned long)usi.ticks;
    else
