/* L8 - SPINLOCKS.
 *
 * Two things students always get wrong:
 *  1. Interrupts MUST be off while holding a lock. If a timer interrupt fires
 *     and the handler grabs the same lock, that hart deadlocks against itself.
 *  2. The memory FENCE is not decoration. Without it the CPU may reorder the
 *     critical section outside the lock.
 * push_off/pop_off nest, so acquiring two locks and releasing them in order
 * does not re-enable interrupts too early.
 */
#include "types.h"
#include "defs.h"
#include "hal.h"
