/* ELF LOADER - exec() replaces the current address space with a program
 * loaded FROM LUITFS. namei finds the inode; readi pulls the bytes; the ELF
 * program headers say where they go.
 *
 * Two things students get wrong, both graded:
 *  1. p_memsz > p_filesz. The difference is the program's .bss and MUST be
 *     zeroed. Skip it and the user program sees garbage in its globals.
 *  2. Permissions. A segment that is not executable must NOT be mapped X.
 *     Mapping everything RWX is a real security hole; a hidden test checks it.
 * A malformed ELF must be REJECTED, not executed - and a failed exec must
 * leave the CALLER intact: nothing old is destroyed until the new image is
 * fully built (the "commit point" below).
 */
#include "types.h"
#include "defs.h"
#include "elf.h"
...
int exec(char *path, char **argv)
{
    ...
    /* PROVIDED Lab 3 lifecycle rule: exec keeps the same info page. */
    if (usysinfo_remap(pp, newpt) < 0) goto bad;

    pp->pagetable = newpt;
    pp->sz        = newsz;
    pp->heapbase  = newsz;             /* sbrk() manages memory above the stack */
    pp->tf->epc   = eh.entry;
    pp->tf->sp    = sp;
