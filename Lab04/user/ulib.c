...
/* ---- Completed Lab 3 dependency: shared-information-page fast accessors ---- */
...
u_snapshot(struct usysinfo *out)
{
    ...
    volatile struct usysinfo *u = usi_page();
    for (;;) {
        uint32 a = u->seq;
        if (a & 1U)
            continue;
        ...
        out->hart          = u->hart;
        out->ticks         = u->ticks;
        out->syscall_count = u->syscall_count;
        out->ctxsw_count   = u->ctxsw_count;
        out->state_gen     = u->state_gen;
        __sync_synchronize();

        uint32 b = u->seq;
        if (a == b && !(b & 1U)) {
            out->seq = b;
            return out->version == USYSINFO_VERSION ? 0 : -1;
        }
    }
}
