#!/usr/bin/env python3
"""Create and verify the exact CS3106L Lab 4 submission ZIP."""
...
FILES = ["kernel/proc.c", "kernel/vm.c", "kernel/trap.c"]


def main():
    if len(sys.argv) != 2 or not re.fullmatch(r"[A-Za-z0-9_-]{5,24}", sys.argv[1]):
        print("usage: python3 tools/package_lab04.py YOUR_ROLL_NUMBER", file=sys.stderr)
        return 2

    roll = sys.argv[1]
    check = subprocess.run([sys.executable, str(ROOT / "tools/lab04_check.py")], cwd=ROOT)
    if check.returncode:
    ...
    for n in expected:
        print("  " + n)
    print("Upload this ZIP. Do not add a report, screenshot, or any other file.")
    return 0
