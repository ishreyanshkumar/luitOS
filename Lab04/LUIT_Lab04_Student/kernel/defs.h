...
int         uvmcopy(pagetable_t old, pagetable_t new, uint64 sz);   /* eager copy; Lab 5 makes this COW */
int         lazy_alloc_page(struct proc *p, uint64 va);             /* Lab 4 */
...
struct proc {
    ...
    struct proc    *parent;
    uint64          kstack;
    uint64          sz;             /* current process break */
    uint64          heapbase;       /* lowest VA that sbrk() may manage */
    ...
    /* Lab 3: one read-only shared page and its published counters. */
    struct usysinfo *usysinfo;
...
/* ---- Lab 2 tracing (completed dependency) ---- */
