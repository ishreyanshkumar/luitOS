> **Lab 4 submission note:** The Lab 4 handout and `tools/package_lab04.py` are authoritative for this graded lab. The Lab 4 ZIP contains exactly `kernel/proc.c`, `kernel/vm.c`, and `kernel/trap.c`; do **not** add an `LLM_LOG.md` or any other file to the Lab 4 ZIP. The general policy below remains course guidance.
...
| 2 trace | generating the tracing recorder/control implementation | asking why the recorder runs after dispatch is GREEN |
| 3 usysinfo | generating `usysinfo_update`, `u_snapshot`, `u_getpid`, or `u_uptime` graded code | seqlock concepts, barrier semantics, GDB inspection, and reviewing code you already wrote are GREEN/AMBER as defined above |
| 4 lazy | generating the K1-K4 lazy-allocation implementation | page-fault concepts, Sv39/PTE questions, and reviewing code you already wrote are GREEN/AMBER as defined above |
