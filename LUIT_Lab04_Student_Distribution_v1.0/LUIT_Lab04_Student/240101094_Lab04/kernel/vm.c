...
/* ---------- Lab 4: fault-driven heap support ---------- */

int
lazy_alloc_page(struct proc *p, uint64 va)
{
    /* TODO-BEGIN K2
     * Materialize exactly one lazily reserved heap page.
     *
     * Return 0 only when a new zero-filled user page has been installed.
     * Return -1 for an invalid address, an address outside the lazy heap,
     * an already-mapped address, allocation failure, or mapping failure.
     *
     * The valid lazy interval is [p->heapbase, p->sz).  Page-align the
     * faulting address with PGROUNDDOWN().  The installed leaf must be
     * user-readable and user-writable, but not executable.  Flush stale
     * translations after changing the current process page table.
     *
     * Keep all Lab 4 K2 code between these markers.
     */
    if (va < p->heapbase || va >= p->sz)
        return -1;
...
/* Resolve one user virtual page for a kernel copy operation.  The supplied
 * starter behavior below is the pre-Lab-4 policy: missing mappings are simply
 * rejected.  K4 extends this policy only for valid lazy pages belonging to the
 * current process. */
static uint64
user_resolve_pa(pagetable_t pt, uint64 va, int need_write)
{
    /* TODO-BEGIN K4
     * Make kernel/user copying lazy-aware without weakening pointer checks.
     *
     * Required behavior:
     *   1. Reject addresses at/above USER_TOP.
     *   2. Walk the PTE and accept it only when PTE_V and PTE_U are set.
     *   3. If the PTE is absent/invalid, materialize it only when `pt` is the
     *      current process's own page table and lazy_alloc_page() accepts va.
     *      Then walk the PTE again.
     *   4. If need_write is non-zero, require PTE_W.
     *   5. Return the physical page base on success, or 0 on failure.
     *
     * This one helper is used by copyout(), copyin(), and copyinstr().  Do not
     * make arbitrary unmapped user addresses valid.
     *
     * Keep all Lab 4 K4 code between these markers.
     */
    if (va >= USER_TOP)
        return 0;
...
int copyout(pagetable_t pt, uint64 dstva, char *src, uint64 len)
{
    while (len > 0) {
        ...
        uint64 pa0 = user_resolve_pa(pt, va0, 1);
        if (pa0 == 0) return -1;
...
int copyin(pagetable_t pt, char *dst, uint64 srcva, uint64 len)
{
    while (len > 0) {
        ...
        uint64 pa0 = user_resolve_pa(pt, va0, 0);
        if (pa0 == 0) return -1;
