...
void usertrap(void)
{
    ...
    if (scause == 8) {               /* ecall: a system call */
        ...
        /* TODO-BEGIN K3
         * Resolve only legitimate lazy-heap load/store faults.
         *
         * r_stval() gives the faulting virtual address.  Call the Lab 4 helper
         * lazy_alloc_page(p, va).  If it succeeds, return normally from the
         * trap path so that sret retries the same user instruction.  If it
         * fails, preserve the old behavior: report the fault and kill only the
         * offending process.  Do not repair guard-page, out-of-range, or
         * protection faults.
         *
         * Keep all Lab 4 K3 code between these markers.
         */
        uint64 va = r_stval();
