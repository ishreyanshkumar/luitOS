/* PIPES - a PIPESIZE-byte ring buffer with blocking semantics.
 *
 * THE SEMANTICS EVERY UNIX PROGRAM DEPENDS ON (all tested):
 *  - read on empty pipe: BLOCKS while any write end is open
 *  - read on empty pipe with NO write ends: returns 0  <- this IS "EOF"
 *  - write on full pipe: BLOCKS while any read end is open
 *  - write with NO read ends: returns -1 (real Unix also raises SIGPIPE)
 * The shell's `cat a | wc` deadlocks in Lab 1 for every student who forgets
 * to close the unused write end in the reader child - because "any write end
 * is open" includes the one THEY are holding.
 *
 * LOCKING: one spinlock per pipe guards everything. Note the wakeup targets:
 * readers sleep on &pi->nread, writers on &pi->nwrite - two channels, so a
 * writer wakes readers, never other writers.
 */
#include "types.h"
#include "defs.h"
