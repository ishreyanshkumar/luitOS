...
/* OpenSBI does not guarantee that hart 0 is the cold-boot/payload hart.
 * In particular, with SMP QEMU the elected boot hart can be 1 (or another
 * hart).  Elect the first hart that enters the payload as Luit's boot hart.
 * Secondary harts started later through SBI HSM will observe this choice. */
...
{
    /* The firmware chooses the initial payload hart.  Do not assume it is 0.
     * Atomic election also keeps this correct on firmware that releases more
     * than one hart into the payload concurrently. */
    int am_boot_hart = __sync_bool_compare_and_swap(&boot_hart, -1, (int)hartid);

    if (am_boot_hart) {
        /* --- boot hart (whichever hart OpenSBI selected) --- */
        ...
        procinit();
        trace_init();       /* completed Lab 2 dependency */
        kvminit();
         ...
         * QEMU virt exposes contiguous hart IDs 0..ncpu-1, but the initial
         * OpenSBI payload hart is not guaranteed to be hart 0. */
