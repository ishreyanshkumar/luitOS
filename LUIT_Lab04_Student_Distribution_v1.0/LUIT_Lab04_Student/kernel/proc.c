...
{
    /* USYSINFO_VA is above p->sz; its backing page must be removed first. */
    usysinfo_free(p);
    ...
    p->sz = 0; p->heapbase = 0; p->pid = 0; p->parent = 0;
...
void userinit(void)
{
    ...
    p->sz       = 2 * PGSIZE;
    p->heapbase = p->sz;               /* heap begins above the initial stack */
...
{
    /* TODO-BEGIN K1
     * Convert sbrk() from eager allocation to lazy reservation.
     *
     * The supplied starter body below is the old eager implementation so that
     * the checkpoint still boots and passes the earlier-course regressions.
     * Replace it with a correct lazy implementation.
     *
     * Required behavior:
     *   - n > 0: change p->sz only; do not allocate a physical page and do not
     *     install a PTE. Reject overflow and growth into USYSINFO_VA.
     *   - n < 0: never shrink below p->heapbase. Unmap/free only complete pages
     *     that leave the heap. Missing PTEs are legitimate lazy holes.
     *   - n == 0: succeed without changing the address space.
     *   - p->heapbase is supplied by the checkpoint and must not be changed.
     *
     * Keep all Lab 4 K1 code between these markers.
     */
    ...
    if (n > 0) {
        ...
        if (newsz < sz || newsz > USYSINFO_VA)
            return -1;
        p->sz = newsz;
        return 0;
    } else if (n < 0) {
        uint64 dec = (uint64)(-(int64)n);
        if (dec > sz || sz - dec < p->heapbase)
            return -1;
        uint64 newsz = sz - dec;
    ...
    for (;;) {
        ...
        for (struct proc *p = proc; p < &proc[NPROC]; p++) {
            ...
            if (p->state == RUNNABLE) {
                ...
                /* Lab 3 publication point: both counters advance together. */
                p->ctxsw_count++;
                p->state_gen++;
                usysinfo_update(p);
