...
/* Lab 5: COPY-ON-WRITE fork. Share parent pages read-only + PTE_COW; clear
 * PTE_W in both; copy lazily in cowfault() on write. */
int uvmcopy(pagetable_t old, pagetable_t new, uint64 sz)
{
    for (uint64 i = 0; i < sz; i += PGSIZE) {
        ...
        palloc_ref_inc((void *)pa);
    }
    sfence_vma();   /* parent PTEs went read-only: flush stale writable TLB */
    return 0;
}

/* Resolve a COW fault: sole owner -> flip writable; shared -> private copy.
 * CRITICAL: sfence_vma() after each PTE change or a stale RO TLB entry
 * corrupts memory far from the fault (Ch 3 'works until it doesn't'). */
