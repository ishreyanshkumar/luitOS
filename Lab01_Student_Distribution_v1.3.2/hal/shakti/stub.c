/* SHAKTI HAL BACKEND - Lab 12 territory.
 *
 * This backend COMPILES (CI builds HAL=shakti on every push) but every entry
 * point panics: it is a contract to fill in, not working hardware support.
 * We do not invent register maps. When your team brings up a real SHAKTI
...
{
    sbi_putchar('\n');
    for (const char *s = "shakti hal: not implemented: "; *s; s++) sbi_putchar(*s);
