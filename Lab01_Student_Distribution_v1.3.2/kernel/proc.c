 ...
 * procstat() syscall and the `ps` / `pstree` utilities. This is a safe scan,
 * not a globally atomic transaction: each slot is copied while protected, but
 * different slots may describe slightly different instants.
 *
 * Parent pointers are protected by wait_lock. The established lock order in
 * fork/exit/wait is wait_lock -> p->lock, so procstat follows the same order.
 * copyout() is performed after releasing both locks because it may fault and
 * must not run while process-table locks are held. */
