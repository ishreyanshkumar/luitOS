...
/* Lab 9: rename(old, new) - atomically move a name. The new link is created
 * BEFORE the old entry is removed, so a crash leaves the file reachable under
 * at least one name (never lost). nlink is bumped for the new link and dropped
 * for the old, netting zero. Refuses to rename directories across parents (would
 * need ".." fixups) and refuses if `new` already exists. */
uint64 sys_rename(void)
{
    char oldp[MAXPATH], newp[MAXPATH], newname[DIRSIZ], oldname[DIRSIZ];
    ...
    ilock(ip);
    ip->nlink++;                      /* the soon-to-exist new link */
    iupdate(ip);
    iunlock(ip);

    /* create the new directory entry */
    struct inode *newdp = nameiparent(newp, newname);
    if (newdp == 0) goto undo;
    ilock(newdp);
    if (newdp->dev != ip->dev || dirlink(newdp, newname, ip->inum) < 0) {
        iunlockput(newdp);
        goto undo;
    }
    iunlockput(newdp);
    ...
    struct inode *olddp = nameiparent(oldp, oldname);
    if (olddp == 0) goto drop;
    ilock(olddp);
    uint32 off;
    struct inode *found = dirlookup(olddp, oldname, &off);
    if (found == 0) { iunlockput(olddp); goto drop; }
    iput(found);
    struct dirent de;
    ...
    /* drop the old link's nlink (we added one for new, removed the old entry) */
    ilock(ip);
    ip->nlink--;
    iupdate(ip);
    iunlockput(ip);
...
drop:
    /* new link created but old removal failed: leave the extra nlink? No - undo
     * the increment we made, since the old entry still exists. */
undo:
    ilock(ip);
    ip->nlink--;
    iupdate(ip);
