...
void fsinit(int dev)
{
    ...
    printf("luitfs: %d blocks, %d inodes, data at block %d, log at %d (%d blocks)\n",
           sb.size, sb.ninodes, sb.datastart, sb.logstart, sb.nlog);
    log_init(dev);                 /* Lab 11: init journal + recover */
}
