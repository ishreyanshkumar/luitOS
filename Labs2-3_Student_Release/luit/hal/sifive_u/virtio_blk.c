...
int hal_block_init(void)
{
    ...
    *R(VIRTIO_MMIO_STATUS) = status;
    ...
    /* FEATURE NEGOTIATION: take what the device offers, strike what we
     * cannot honour. Accepting VIRTIO_BLK_F_RO by accident gives you a
     * disk that silently ignores writes - a hidden test would find it. */
