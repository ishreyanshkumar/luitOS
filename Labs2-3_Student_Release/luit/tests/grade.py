#!/usr/bin/env python3
# BrahmaputraOS / Luit -- public grading harness (Teaching Base v1.0, Patch 01)
#
...
def run_qemu(cpus, mem, script, timeout, until=None, image=None):
...
def persistence_test(cpus, mem, timeout=40):
    """Write a file, POWER OFF, boot again on the same image, read it back.
    This is the Tier-2 'survives power loss for completed writes' claim."""
    img = fresh_image("fs.persist.img")
    run_qemu(cpus, mem, [(6, "echo survives > p.txt"), (2, "")],
             timeout, until=None, image=img)
    tr = run_qemu(cpus, mem, [(6, "cat p.txt")], timeout,
                  until=["survives", "cannot open"], image=img)
    return check("persistence: file survives reboot",
                 tr, ["survives"], PANIC_MARKERS + ["cannot open"])
